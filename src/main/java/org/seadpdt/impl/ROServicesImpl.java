...
public class ROServicesImpl extends ROServices {
	...
	private List<String> getOrganizationforPerson(String pID) {
		...
		if (profile != null) {
			...
			Document profileDoc = PeopleServicesImpl.retrieveProfile(profile
                    .getIdentifier());
			...
			if (profileDoc == null) {
				...
				new PeopleServicesImpl().registerPerson("{\"provider\": \""
                        + profile.getProvider() + "\", \"identifier\":\""
                        + profile.getIdentifier() + "\" }");
			}
			profileDoc = PeopleServicesImpl.retrieveProfile(profile.getIdentifier());
